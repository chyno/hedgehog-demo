...
class Authentication {
  ...
  static async createKey (password, ivHex) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof window !== 'undefined' && window && window.Worker) {
        const worker = Utils$1.WebWorker(authWorker.toString());
        ...
        crypto.scrypt(passwordBuffer, ivBuffer, dkLen, { N, r, p, maxmem }, (err, derivedKey) => {
          if (err) {
            ...
            let keyBuffer = Utils$1.bufferFromHexString(keyHex);
  ...
  static decrypt (ivBuffer, keyBuffer, cipherTextHex) {
    ...
    let cipherText = BufferSafe.from(Utils$1.bufferFromHexString(cipherTextHex));
...
if (typeof window === 'undefined' || window === null) {
  const LocalStorage = require$$0.LocalStorage;
